...
```{r}
taxi.k.means.statitcs <- data.frame(1:200)
colnames(taxi.k.means.statitcs) <- c("k")
...
taxi.k.means.statitcs <- apply(taxi.k.means.statitcs,1, taxi.collect.kmean.statistics)

# Set label
taxi.k.means.statitcs <- as.data.frame(t(taxi.k.means.statitcs))
...
Taxi.Withingness.And.Betweeness.by.different.K.Closer <- function(){
  closer <- taxi.k.means.statitcs[3:20,]
...
```{r}
t.k.s.d <- kmeans(trip.data[,8:9],6, iter.max=200, algorithm="MacQueen")
